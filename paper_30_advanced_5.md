...
```python
s = "abcdef"
result = [s[i:j] for i in range(len(s)) for j in range(i+2, len(s)+1) if j - i == 2]
print(result)
```
a) `["ab", "bc", "cd", "de", "ef"]`
b) `["abcdef"]`
...
```python
data = [1, 2, [3, 4], [5, [6, 7]]]
flat = [x for item in data for x in (item if isinstance(item, list) else [item])]
print(flat)
```
a) `[1, 2, 3, 4, 5, 6, 7]`
b) `[1, 2, 3, 4, 5, [6, 7]]`
...
```python
curry_add = lambda a: lambda b: lambda c: a + b + c
...
expr = [("+", 2, 3), ("*", 4, 5), ("-", 10, 7)]
result = [ops[op](a, b) for op, a, b in expr]
print(result)
```
a) `[5, 20, 3]`
b) `["+", "*", "-"]`
...
```python
deep_get = lambda d, *keys: d if not keys else deep_get(d.get(keys[0], {}), *keys[1:])
data = {"a": {"b": {"c": 42}}}
...
print(interleave(words[0], words[1]))
```
