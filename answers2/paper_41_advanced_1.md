...
```python
words = ["abc", "de", "fghi"]
result = [c for w in words for c in w if c > "c"]
print(result)
```
a) `["abc", "de", "fghi"]`
b) `["d", "e", "f", "g", "h", "i"]`
...
```python
nums = [1, 2, 3, 4, 5]
result = [[x * y for y in nums] for x in range(1, 3)]
print(result[0])
```
a) `[1, 2, 3, 4, 5]`
b) `[2, 4, 6, 8, 10]`
...
```python
s = "hello"
result = [s[:i] + s[i+1:] for i in range(len(s))]
print(result)
```
...
```python
data = [("A", [90, 80]), ("B", [70, 60]), ("C", [85, 95])]
result = max(data, key=lambda x: sum(x[1]))
print(result[0])
```
a) `"A"`
b) `"B"`
...
```python
counter = lambda: (lambda c=[0]: (c.__setitem__(0, c[0]+1), c[0])[-1])
count = counter()
...
```python
transform = lambda d, fn: {k: fn(v) for k, v in d.items()}
data = {"a": 1, "b": 2, "c": 3}
print(transform(data, lambda x: x ** 2))
```
