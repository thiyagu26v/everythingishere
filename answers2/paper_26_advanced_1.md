...
```python
result = [x for x in [None, 0, "", False, 1, "hi", []] if x]
print(result)
```
a) `[None, 0, "", False, 1, "hi", []]`
b) `[1, "hi"]`
...
```python
s = "programming"
result = {c: s.count(c) for c in s if s.count(c) > 1}
...
```python
data = [[3, 1], [1, 4], [2, 2]]
result = sorted(data, key=lambda x: sum(x))
print(result)
```
...
print(toggle(True), toggle(False))
```
a) `True True`
b) `False True`
c) `True False`
...
result = reduce(lambda a, b: f"({a}+{b})", ["a", "b", "c", "d"])
print(result)
```
a) `"a+b+c+d"`
b) `"(((a+b)+c)+d)"`
...
result = sorted(data.items(), key=lambda x: len(x[1]), reverse=True)
