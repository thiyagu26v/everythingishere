...
result = {k: sum(v) for k, v in merged.items()}
print(result)
```
a) `{"a": 4, "b": 6, "c": 5}`
b) `{"a": [1, 3], "b": [2, 4], "c": [5]}`
...
```python
data = "aababcabcd"
result = {c: data.index(c) for c in sorted(set(data))}
print(result)
```
a) `{"a": 0, "b": 2, "c": 5, "d": 9}`
b) `{"a": 1, "b": 3, "c": 6, "d": 10}`
...
result = [x for x in nums if all(x % d != 0 for d in range(2, x))]
print(result)
```
...
anagrams = [v for v in groups.values() if len(v) > 1]
print(anagrams)
```
...
```python
encode = lambda s: [(c, len(list(g))) for c, g in __import__("itertools").groupby(s)]
...
```python
safe_get = lambda d, key, default=None: d.get(key, default)
data = {"name": "Ravi", "age": 25}
print(safe_get(data, "city", "Unknown"))
```
a) `None`
b) `"Unknown"`
c) `"city"`
...
```python
transform = lambda lst: list(map(lambda x: (x, x**2, x**3), lst))
result = transform([2, 3])
print(result[1])
```
a) `(2, 4, 8)`
b) `(3, 9, 27)`
...
```python
flatten_dict = lambda d, prefix="": {f"{prefix}{k}" if not isinstance(v, dict) else kk: vv for k, v in d.items() for kk, vv in (flatten_dict(v, f"{prefix}{k}.") if isinstance(v, dict) else {f"{prefix}{k}": v}).items()}
...
```python
data = [(1, "a"), (2, "b"), (3, "c")]
swap = lambda pairs: list(map(lambda p: (p[1], p[0]), pairs))
print(swap(data))
```
