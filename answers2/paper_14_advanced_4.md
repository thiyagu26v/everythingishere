...
```python
data = [("a", 1), ("b", 2), ("c", 3)]
result = dict([(v, k) for k, v in data])
print(result)
```
a) `{"a": 1, "b": 2, "c": 3}`
b) `{1: "a", 2: "b", 3: "c"}`
...
```python
nums = [1, 2, 3, 4, 5]
result = [a * b for a, b in zip(nums, nums[1:])]
print(result)
```
a) `[1, 4, 9, 16, 25]`
b) `[2, 6, 12, 20]`
...
result = {w[0]: [x for x in words if x[0] == w[0]] for w in words}
print(len(result["a"]))
```
...
```python
counter = lambda lst: {x: lst.count(x) for x in set(lst)}
...
```python
data = ["3", "1", "10", "2", "20"]
result = sorted(data, key=lambda x: int(x))
print(result)
```
a) `["1", "10", "2", "20", "3"]`
b) `["1", "2", "3", "10", "20"]`
...
```python
items = [{"name": "A", "qty": 5}, {"name": "B", "qty": 0}, {"name": "C", "qty": 3}]
in_stock = list(filter(lambda x: x["qty"] > 0, items))
print([x["name"] for x in in_stock])
```
a) `["A", "B", "C"]`
b) `["A", "C"]`
c) `["B"]`
...
result = compose(lambda x: x + "!", lambda x: x.upper())
print(result("hello"))
```
...
```python
matrix = [[1, 2, 3], [4, 5, 6]]
flat = list(map(lambda row: list(map(lambda x: x ** 2, row)), matrix))
print(flat)
```
