...
```python
result = [x for x in range(100) if x % 15 == 0]
print(result)
```
...
```python
vals = ["10", "20", "30"]
result = list(map(lambda x: int(x) * 2, vals))
print(result)
```
a) `["1010", "2020", "3030"]`
b) `[20, 40, 60]`
...
```python
apply = lambda fn, lst: [fn(x) for x in lst]
result = apply(lambda x: x.strip(), ["  hi  ", " world ", "python"])
print(result)
```

**Q10 (Open-Ended):** What is the output?
```python
inventory = {"pen": 5, "book": 0, "bag": 3, "eraser": 0}
