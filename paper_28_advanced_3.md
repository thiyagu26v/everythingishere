...
b) `"Python"`
c) `"Pytho"`
d) `["P", "Py", "Pyt", "Pyth", "Pytho", "Python"]`

**Q2 (MCQ):** What is the output?
```python
result = {k: len(v) for k, v in {"fruits": ["apple", "banana"], "vegs": ["carrot"]}.items()}
print(result)
```
a) `{"fruits": ["apple", "banana"], "vegs": ["carrot"]}`
b) `{"fruits": 2, "vegs": 1}`
c) `[2, 1]`
d) `3`
...
```python
safe = lambda fn, default: lambda *args: fn(*args) if args else default
add = safe(lambda a, b: a + b, 0)
print(add(3, 4))
```
...
result = reduce(lambda a, b: a + ", " + b["name"], data[1:], data[0]["name"])
print(result)
```
