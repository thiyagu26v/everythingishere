...
```python
result = [i for i in range(10) if i % 2 == 0 and i % 4 == 0]
print(result)
```
a) `[0, 2, 4, 6, 8]`
b) `[0, 4, 8]`
...
```python
text = "aAbBcC"
result = [c for c in text if c.islower()]
print(result)
```
a) `["a", "A", "b", "B", "c", "C"]`
b) `["a", "b", "c"]`
...
```python
words = ["Hi", "By", "No"]
result = ["".join(reversed(w)) for w in words]
print(result)
```
...
```python
calc = lambda op, a, b: a + b if op == "+" else a - b if op == "-" else a * b
print(calc("*", 5, 4))
```
a) `9`
b) `1`
...
```python
data = [4, 7, 2, 9, 1, 6]
result = list(filter(lambda x: 3 <= x <= 7, data))
print(result)
```
a) `[4, 7, 2, 9, 1, 6]`
b) `[4, 7, 6]`
...
result = list(map(lambda s: s.center(10, "-"), strings))
print(result[2])
```
a) `"python----"`
b) `"----python"`
...
```python
chain = lambda x: lambda y: lambda z: x + y + z
print(chain(1)(2)(3))
```
...
```python
pairs = [("a", 1), ("b", 2), ("c", 3)]
result = dict(map(lambda p: (p[0], p[1] * 10), pairs))
print(result)
```
