...
```python
result = [chr(x) for x in range(65, 70)]
print(result)
```
a) `[65, 66, 67, 68, 69]`
b) `["A", "B", "C", "D", "E"]`
...
```python
s = "abcde"
result = [s[i:i+2] for i in range(len(s)-1)]
print(result)
```
a) `["a", "b", "c", "d", "e"]`
b) `["ab", "bc", "cd", "de"]`
...
result = [v["x"] * 2 for v in nested.values()]
print(result)
```
a) `[1, 2, 3]`
b) `[2, 4, 6]`
...
```python
apply_all = lambda x, *funcs: [f(x) for f in funcs]
result = apply_all(5, lambda x: x+1, lambda x: x*2, lambda x: x**2)
print(result)
```
a) `[6, 10, 25]`
b) `[5, 5, 5]`
...
```python
data = ["hello", "hi", "hey", "howdy", "hola"]
groups = {}
for w in data:
    key = (lambda x: len(x))(w)
    groups.setdefault(key, []).append(w)
print(sorted(groups.keys()))
```
a) `["hello", "hi", "hey", "howdy", "hola"]`
b) `[2, 3, 4, 5]`
c) `[2, 3, 4, 5]`
d) `[5, 2, 3, 5, 4]`
...
```python
process = lambda lst: list(map(lambda x: x.strip().lower(), lst))
...
word = "Hello"
results = [v(word) for v in validators]
print(results)
```
